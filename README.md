...
- **在线逐字覆盖率**：酷狗关键词改为接口认可的「艺术家 - 曲名」并按评分排序（此前几乎从不命中）；QQ 改走客户端取词接口解加密 QRC（非标准 3DES 自实现、`des` 依赖移除，本地 `.qrc` 同修）；网易云拿到 yrc 不再丢译文 / 音译，副轨按起点一对一对齐；KRC 逐字罗马音作音译；候选按能力排序并标「逐字 / 逐行」「译文」「音译」徽标；本地文件名平台 ID 双目录查 AMLL 并核对文件头。
- **可听位置定位**：进度事件附带输出延迟，三处显示的行定位、逐字锚点与间奏判定据此回拨；进度条与跳转仍用引擎位置。
- **歌词模型重构**：`LyricDocument`（来源身份、同步粒度、文件 offset）、毫秒整数、结束时间可空并标记推导、译文 / 音译结构化字段、角色与对唱标记；曲库清单 1 → 2 自动迁移；AMLL 查找键随文档落盘（修复重启即丢）。
- **角色渲染与多活动区间**：TTML 和声独立成行挂主句并逐字填色，制作信息成块弱化，对唱不再当双语；对唱重叠时同时高亮，滚动跟随最早开始且仍在唱的主句，任务栏只展示主句。
- **固定来源与显示选项**：手动导入 / 明确应用的歌词不被重新导入与目录匹配替换，右键可固定 / 取消固定，曲目信息显示来源；「显示制作信息」「忽略歌词文件里的 offset」由后端回传前投影，任务栏一致，进配置导出。
- **回归覆盖**：前端 281 → 309，Rust 328 → 379。
