...
性能优化的范围、复测数据与验证边界见 [优化修复记录](docs/OPTIMIZATION-FIXES-2026-09-13.md)，原始基线见 [优化审查报告](docs/OPTIMIZATION-REVIEW-2026-09-13.md)；v0.5.11 的缺陷修复记录见 [修复验证记录](docs/BUG-FIXES-2026-09-12.md)。
...
- **歌词设置（v0.6.0）**：设置 → 歌词设置提供歌词源优先级（自动 / 网易云 / 酷狗 / QQ 优先）、繁体中文转换（新获取与导入的歌词生效）、在线 TTML 逐字歌词（AMLL TTML DB，默认开启，命中候选排最前）、译文与音译显示开关、歌词排除规则（关键词 / Rust 正则，后端匹配打标、主窗口与任务栏一致，仅影响显示）、本地歌词目录（切歌无歌词时按「艺术家 - 曲名」匹配，文件名里的网易云 ID 复用为 AMLL 查找键）以及任务栏歌词条的三项开关。TTML DB 地址可用预设社区镜像或自定义模板（`{dir}` / `{id}` / `%s` 占位），自定义模式限公网 HTTPS 域名且禁止重定向。
- **逐字歌词高亮**：TTML 与增强型 LRC（行内 `<mm:ss.xxx>` 标签，如 LDDC 导出）来源的当前行在侧栏、沉浸播放与任务栏歌词条按音节从左到右填色，配平滑时钟；行级歌词沿用打字机动画。
...
截至 v0.5.12，自动化回归为前端 **243 项**、Rust **297 项**，合计 **540 项**。覆盖批量删除与重试、流媒体缓存文件处理、沉浸播放入口与键盘操作、歌词定位与在线匹配、进度拖动、队列检索，以及分析冻结、隐藏窗口和切歌时的帧生命周期。上述检查也会在 GitHub Release 工作流中作为发布门禁执行。

沉浸界面已在浏览器中检查 960×600、1280×720 的两种模式，以及 1920×1080 的歌词模式；声场绘制完成 100%、125%、150%、200% 缩放下的离屏对比，使用同一组合成 PCM 经实际 Rust 分析引擎抽样。测试范围及实体设备验证边界见 [优化修复记录](docs/OPTIMIZATION-FIXES-2026-09-13.md) 与 [修复验证记录](docs/BUG-FIXES-2026-09-12.md)；真实声卡播放、设备切换与用户曲库迁移尚未桌面实测。
...
- **增强型 LRC 与本地歌词目录**：解析 `<mm:ss.xxx>` 行内逐字标签；可设置歌词目录，切歌无歌词时按「艺术家 - 曲名」匹配，文件名网易云 ID 记为 AMLL 查找键直取 TTML。
- **修复**：逐字行因 `currentColor` 随文字透明导致原文不可见。
- **回归覆盖**：前端 243 → 264，Rust 库测试 200。

### v0.5.12

新增沉浸播放与曲库批量删除，落地 2026-09-13 优化审查的 16 项建议，曲库存储改为快照格式。详见 [v0.5.12 发布说明](docs/releases/v0.5.12.md)。
...
详细说明见 [v0.5.11 发布说明](https://github.com/aoiasuka/SeraphAudioPlayer/releases/tag/v0.5.11)，触发条件与修复证据见 [BUG 审查报告](docs/BUG-AUDIT-2026-09-12.md)。
