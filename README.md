...
- **声学分析套件**：侧栏「系统 → 声学分析」提供五仪表实时分析页——响度计（EBU R128 / ITU BS.1770-4 语义：M/S/I、LRA、真峰≈、目标偏差标尺）、Peak+RMS 电平表（峰值保持、削波印章）、立体声声场仪（极坐标/李萨如散点 + 相关度表）、96 频点对数频谱（峰值保持、悬停游标读数）、频谱瀑布（墨线山脊 / 纸墨热图双模式）；全部由播放输出的真实样本驱动，~30fps 轮询单点计算，不占音频线程。
