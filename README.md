...
当前代码版本：**v0.6.0**。该版本新增**歌词设置**：在线歌词源优先级、繁体中文转换、AMLL TTML 逐字歌词（译文 / 音译 / 逐字高亮）、可切换预设或自定义模板的 TTML DB 地址，以及歌词排除规则，详见 [版本说明](docs/releases/v0.6.0.md)。v0.5.12 引入沉浸播放双模式、曲库多选与全部删除及一轮性能优化。
...
- **逐字歌词高亮**：TTML、增强型 LRC（行内 `<mm:ss.xxx>` 标签，如 LDDC 导出）与 QRC / KRC / YRC 来源的当前行在侧栏、沉浸播放与任务栏歌词条按音节从左到右填色，配平滑时钟；行级歌词沿用打字机动画。
...
歌词行模型为 `{time, text}` 加可选的 `end` / `words` / `translation` / `roman`：LRC、QRC、KRC、YRC 只填前两项，译文以相邻同时间戳行表示；AMLL TTML 填满逐字音节与译文 / 音译字段。三处显示组件同时渲染两种译文形态。排除规则在显示层过滤，不改写曲库；繁体转换在歌词写入前完成。
...
截至 v0.6.0，自动化回归为前端 **281 项**、Rust **328 项**，合计 **609 项**。覆盖歌词解析（LRC / 增强 LRC / QRC / KRC / YRC / TTML）、排除规则与隐藏句定位、AMLL 地址守卫与缓存、批量删除与重试、流媒体缓存文件处理、沉浸播放入口与键盘操作、歌词定位与在线匹配、进度拖动、队列检索，以及分析冻结、隐藏窗口和切歌时的帧生命周期。上述检查也会在 GitHub Release 工作流中作为发布门禁执行。
...
新增歌词设置：歌词源优先级、繁体中文转换、AMLL TTML 逐字歌词与歌词排除规则。详见 [v0.6.0 发布说明](docs/releases/v0.6.0.md)。
...
以下以从 0.5.12 升级到 0.6.0 为例；版本已同步时无需重复执行 `bump`：

```bash
npm run bump 0.6.0
git add -A
git commit -m "release: v0.6.0 歌词设置与 AMLL TTML 逐字歌词"
git tag -a v0.6.0 -m "Seraph Audio Player v0.6.0"
