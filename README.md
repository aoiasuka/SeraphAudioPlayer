...
当前代码版本：**v0.5.11**。该版本修复了启动静音、重缓存与删除竞态、随机历史回退、配置恢复和声学分析等 12 类问题，详见 [版本说明](docs/releases/v0.5.11.md) 与 [修复验证记录](docs/BUG-FIXES-2026-09-12.md)。

v0.5.11 之后的开发版本新增了**沉浸播放双模式**（封面 · 歌词 / 声学分析）、**曲库多选与全部删除**、**流媒体缓存音频文件删除**，并包含队列增量同步、曲库快照存储与恢复、可视化按需计算等性能优化。**这些后续变更尚未发布到 GitHub Releases**，可从当前源码构建体验；2026-09-14 已完成 Windows x64 主程序与 NSIS EXE 安装包的本地构建。

性能优化的范围、复测数据与验证边界见 [优化修复记录](docs/OPTIMIZATION-FIXES-2026-09-13.md)，原始基线见 [优化审查报告](docs/OPTIMIZATION-REVIEW-2026-09-13.md)。
...
4. 使用当前源码构建的版本时，选中曲目后可点击底部播放条左侧的圆形专辑封面进入沉浸播放，在右上角切换「封面 · 歌词」和「声学分析」；点击「收起」或按 `Esc` 返回资料库。
...
默认窗口下会为立体声场增加显示空间；声场画布至少使用两倍像素密度，散点按像素对齐并减轻历史拖影，保持真实的左右声道幅度比例。
...
截至 2026-09-14，自动化回归为前端 **232 项**、Rust **297 项**，合计 **529 项**。新增覆盖批量删除与重试、流媒体缓存文件处理、沉浸播放入口与键盘操作、歌词定位、进度拖动、队列检索，以及分析冻结、隐藏窗口和切歌时的帧生命周期。上述检查也会在 GitHub Release 工作流中作为发布门禁执行。

沉浸界面已在浏览器中检查 960×600、1280×720 的两种模式，以及 1920×1080 的歌词模式；分析绘图使用测试帧验证。此前测试范围及实体设备验证边界见 [优化修复记录](docs/OPTIMIZATION-FIXES-2026-09-13.md) 与 [修复验证记录](docs/BUG-FIXES-2026-09-12.md)，本轮未复测实际声卡播放或安装流程。
...
2026-09-14 已完成 v0.5.11 的本地 Release / NSIS 构建，包含沉浸播放、批量删除，以及完整无滚动条歌词、长标题布局和默认窗口声场清晰度调整；已核对文件生成时间、版本、PE 文件头与 SHA-256。产物为：

| 产物 | 路径 | 大小 |
| --- | --- | --- |
| Windows x64 主程序 | `target/release/seraph-audio-player.exe` | 26.14 MiB |
| EXE 安装包 | `target/release/bundle/nsis/Seraph Audio Player_0.5.11_x64-setup.exe` | 10.40 MiB |

本地构建日志与产物核对记录分别位于 `target/immersive-layout-exe-build-2026-09-14.log` 和 `target/immersive-layout-exe-verification-2026-09-14.json`；`target/` 不随 Git 提交，本次构建目标为 NSIS。

本次显示调整通过 34 项相关前端测试与类型检查，并完成 100%、125%、150%、200% 缩放下的声场离屏绘图对比；整窗浏览器视觉复核因工具自动审批服务返回 403 权限错误未完成。

## 版本记录

### 未发布（2026-09-14：沉浸播放与曲库批量删除）

基于 v0.5.11，版本号未变更，已完成本地 EXE 构建，尚未发布到 GitHub Releases。

- **沉浸播放双模式**：底部封面进入「封面 · 歌词」或「声学分析」，沿用纸色档案风格；同步歌词支持译文、字号和定位，分析支持真实频谱、声场、电平及冻结画面。
- **共用播放状态**：切换与收起保持曲目和进度，播放队列支持检索；隐藏页面让出可视化轮询，切歌丢弃旧帧，支持键盘访问与减少动态效果偏好。
- **曲库批量删除**：多选、全选、全部删除共用确认流程，范围遵循当前列表与检索结果；本地原文件保留，流媒体缓存音频实际删除，失败项保留并可重试。
- **回归与构建**：前端 203 → 232 项、Rust 288 → 297 项，共 529 项；完成沉浸界面的窗口尺寸与交互检查，生成 Windows x64 主程序和 NSIS EXE 安装包。
