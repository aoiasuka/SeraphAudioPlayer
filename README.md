...
当前代码版本：**v0.5.12**。该版本新增**沉浸播放双模式**（封面 · 歌词 / 声学分析）、**曲库多选与全部删除**、**流媒体缓存音频文件删除**与更饱满的立体声场显示，并落地队列增量同步、曲库快照存储与恢复、可视化按需计算等性能优化，详见 [版本说明](docs/releases/v0.5.12.md)。
...
以下以从 0.5.11 升级到 0.5.12 为例；版本已同步时无需重复执行 `bump`：

```bash
npm run bump 0.5.12
git add -A
git commit -m "release: v0.5.12 沉浸播放、曲库批量删除与性能优化轮"
git tag -a v0.5.12 -m "Seraph Audio Player v0.5.12"
git push origin main v0.5.12
```
