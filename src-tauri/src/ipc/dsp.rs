...
fn import_eq_preset_inner(path: &str) -> IpcResult<String> {
    // F-02：只接受 .txt/.json 预设且必须是已存在的普通文件
    let path = validate_import_path(path, &["txt", "json"])?;
    let metadata = std::fs::metadata(&path)
        .map_err(|_| IpcError::not_found(format!("预设文件不存在: {}", path.display())))?;
...
fn export_eq_preset_inner(path: &str, content: &str) -> IpcResult<()> {
    ...
    let target = validate_export_path(path, &["json", "txt"])?;
